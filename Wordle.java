...
{ 
	...
	{
		Scanner fileScanner = FileUtils.openToRead(WORDS5_ALLOWED);
		int numLines = 0, i, targetWordIndex;
		String wordLine;
		String result = "SMART";
		
		while (fileScanner.hasNextLine()){
			wordLine = fileScanner.nextLine();
			//testWord was found in WORDS5_ALLOWED
			if(testWord.equalsIgnoreCase(wordLine)){
				return testWord;
			}
			numLines++;
		}
		...
		//testWord was not found in WORDS5_ALLOWED.
		fileScanner = FileUtils.openToRead(WORDS5_ALLOWED);
		
		targetWordIndex = (int)(Math.random()*numLines);
		for(i=0;i<targetWordIndex; i++){
	...
	{
		Scanner fileCheck = FileUtils.openToRead(WORDS5_ALLOWED);
		while(fileCheck.hasNextLine()){
			if(fileCheck.nextLine().equalsIgnoreCase(possibleWord)){
				return true;
			}
		}
	...
	public void processGuess ( )
	{
		...
		// else if guess is not in words5allowed.txt then print dialog box
			...
			JDialog d = pane.createDialog(null, "Oops!");
	...
	public void checkIfWonOrLost ( )
	{
		...
		else if(!wordGuess[wordGuess.length-1].isEmpty()){
			activeGame = false;
